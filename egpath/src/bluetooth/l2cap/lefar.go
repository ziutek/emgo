...
// LEFAR implements Fragmentation And Recombination component of L2CAP for BLE
// controller.
type LEFAR struct {
	hci   ble.HCI
	rxpay []byte
...
func NewLEFAR(hci ble.HCI) *LEFAR {
	f := new(LEFAR)
	f.hci = hci
	return f
}
...
func (f *LEFAR) ReadHeader() (length, cid int, err error) {
	for {
		pdu, err := f.hci.Recv()
		if err != nil {
			return 0, 0, err
		...
		if pdu.Header()&ble.LLID == ble.L2CAPStart {
			...
			f.rxpay = p[4:]
			f.tord = uint16(length)
			return length, cid, nil
		}
	}
}

// Read reads payload of L2CAP frame. It can read no more than payload length
// returned by ReadHeader. Read returns err = io.EOF if there is no more data
// to read from payload.
func (f *LEFAR) Read(s []byte) (n int, err error) {
	for {
		if n == len(s) {
			return n, nil
		}
...
func (f *LEFAR) WriteHeader(length, cid int) {
	if f.towr != 0 {
...
func (f *LEFAR) WriteString(s string) (n int, err error) {
	if int(f.towr) < len(s) {
