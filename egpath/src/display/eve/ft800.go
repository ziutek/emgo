...
const (
	ACTIVE  HostCmd = 0x00 // Switch from Standby/Sleep/PwrDown to Active mode.
	...
	PD_ROMS HostCmd = 0x49 // Power down individual ROMs (FT81x).

	CLKINT HostCmd = 0x48 // Select PLL internal clock source (default, FT81x).
	CLKEXT HostCmd = 0x44 // Select PLL external clock source.
	CLK48M HostCmd = 0x62 // Switch PLL output to 48 MHz (FT80x, default).
	CLK36M HostCmd = 0x61 // Switch PLL output to 36 MHz (FT80x).
	CLKSEL HostCmd = 0x62 // Select PLL multipler (FT81x).

	CORERST HostCmd = 0x68 // Send reset pulse fot FT8xx core.

	PINDRIVE     HostCmd = 0x70 // Set pins drive strength (FT81x).
	PIN_PD_STATE HostCmd = 0x71 // Set pins state in PwrDown mode.
)
...
// Memory map.
const (
	RAM_G   = 0x000000 // Main graphics RAM (FT80x: 262144 B, FT811: 1048576 B).

	RAM_CMD_FT80x = 0x108000
	RAM_DL  = 0x100000
	RAM_PAL = 0x102000
	RAM_REG = 0x102400
)
