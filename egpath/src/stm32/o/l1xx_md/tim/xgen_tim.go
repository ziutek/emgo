...
func (p *TIM_Periph) OCCS() RMSMCR {
	return RMSMCR{mmio.UM16{&p.SMCR.U16, uint16(OCCS)}}
}
...
func (r *ROR) Bits(mask OR) OR      { return OR(r.U16.Bits(uint16(mask))) }
func (r *ROR) StoreBits(mask, b OR) { r.U16.StoreBits(uint16(mask), uint16(b)) }
func (r *ROR) SetBits(mask OR)      { r.U16.SetBits(uint16(mask)) }
func (r *ROR) ClearBits(mask OR)    { r.U16.ClearBits(uint16(mask)) }
func (r *ROR) Load() OR             { return OR(r.U16.Load()) }
func (r *ROR) Store(b OR)           { r.U16.Store(uint16(b)) }

type RMOR struct{ mmio.UM16 }

func (rm RMOR) Load() OR   { return OR(rm.UM16.Load()) }
func (rm RMOR) Store(b OR) { rm.UM16.Store(uint16(b)) }

func (p *TIM_Periph) TI1RMP() RMOR {
	return RMOR{mmio.UM16{&p.OR.U16, uint16(TI1RMP)}}
}
