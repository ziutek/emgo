...
func enableClock(p *Port, _ bool) {
	rcc.RCC.APB2ENR.AtomicSetBits(rcc.IOPAEN << portnum(p))
	rcc.RCC.APB2ENR.Load() // RCC delay (workaround for silicon bugs).
}

func disableClock(p *Port) {
	rcc.RCC.APB2ENR.AtomicClearBits(rcc.IOPAEN << portnum(p))
}

func reset(p *Port) {
	pnum := portnum(p)
	rcc.RCC.APB2RSTR.AtomicSetBits(rcc.IOPARST << pnum)
	rcc.RCC.APB2RSTR.AtomicClearBits(rcc.IOPARST << pnum)
}
