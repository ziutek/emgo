...
// bytes to provided Drv:
//
//	- first:  with E bit unset, need >= 40 ns
//	- second: with E bit set,   need >= 230 ns
//	- thrid:  with E bit unset, need >= 10 ns
//
// Full E cycle needs >= 500 ns. Display doesn't control proper nible timing
// (Drv implementation must do it).
//
...
type Display struct {
	...
	Cols byte
	Rows byte
	DLs  byte // Shift for D4-D7 data lines.
	E    byte // E line.
	RW   byte // R/W line.
	RS   byte // RS line.
...
func (d *Display) SetAUX() error {
	d.auxst = d.AUX
	_, err := d.Drv.Write([]byte{d.AUX, d.AUX, d.AUX})
	return err
}

func (d *Display) ClearAUX() error {
	d.auxst = 0
	_, err := d.Drv.Write([]byte{0, 0, 0})
	return err
}

func writeByte(d *Display, rs, b byte) error {
	h := (b>>4)<<d.DLs | rs | d.auxst
	l := (b&0xf)<<d.DLs | rs | d.auxst
	buf := []byte{
		h, h | d.E, h,
		l, l | d.E, l,
	}
...
func waitBusy(d *Display) error {
	rba := d.RW | 0xf<<d.DLs | d.auxst
	out := []byte{rba, rba | d.E, rba}
	var in [1]byte
	for {
		if _, err := d.Drv.Write(out[:2]); err != nil {
			return err
		}
		if _, err := d.Drv.Read(in[:]); err != nil {
			return err
		}
		if _, err := d.Drv.Write(out[2:]); err != nil {
			return err
		}
		if _, err := d.Drv.Write(out); err != nil {
			return err
		}
		if (in[0]>>d.DLs)&0x8 == 0 {
			return nil
...
func readBusyAddr(d *Display) (byte, error) {
	...
	if _, err := d.Drv.Read(in[:1]); err != nil {
		return 0, err
	}
	if _, err := d.Drv.Write(out[:1]); err != nil {
		return 0, err
	}
	if _, err := d.Drv.Write(out); err != nil {
		return 0, err
	...
	return (in[0]>>d.DLs)<<4 | (in[1]>>d.DLs)&0xf, nil
}
*/
